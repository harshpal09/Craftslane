...
const HomeAccent = ({route, navigation}) => {
  ...
  getData = async () => {
    ...
    console.log(
      'Profile Page url=>',
      parsed.url +
        'customproductprofile/index&key=' +
        parsed.key +
        '&product_id=' +
        id,
    );
    await axios
      ...
      .then(resp => {
        ...
        setPrice(
          resp.data.body.range_price == ''
            ? resp.data.body.price
            : resp.data.body.range_price,
        );
        ...
        setOptionRequireYesLength(
          resp.data.body.options_require_yes != undefined
            ? resp.data.body.options_require_yes.length
            : 0,
        );
        ...
        if (resp.data.body.options_require_yes != undefined) {
          if (resp.data.body.view_type == '7') {
            ...
            let arr = []
              resp.data.body.personalized_main_options_require_yes.map((item,ind)=>{
                if(ind == 3) arr.push(false);
                else
                arr.push(Array.isArray(item.product_option_value_data_child))
                
                if(!Array.isArray(item.product_option_value_data_child)){
                  setSubDropdownId(item.product_option_value_data_child[resp.data.body.personalized_main_options_require_yes[ind-1].product_option_value[0].option_value_id])
                  // console.log("asdfghjhgfd  ",resp.data.body.personalized_main_options_require_yes[ind-1].product_option_value[0].option_value_id)
                }
              })
              setDropdown(arr);
          }
          setProductOptionValue(
            resp.data.body.options_require_yes.length > 0
              ? resp.data.body.options_require_yes[0].product_option_value
              : [],
          );
          setProductOptionValue2(
            resp.data.body.options_require_yes.length > 1
              ? resp.data.body.options_require_yes[1].product_option_value
              : [],
          );
          setProductOptionValue3(
            resp.data.body.options_require_yes.length > 2
              ? resp.data.body.options_require_yes[2].product_option_value
              : [],
          );
        ...
        if (
          resp.data.body.options_require_yes != undefined &&
          resp.data.body.options_require_no != undefined
        ) {
          setToggle(true);
        }
        if (resp.data.body.options_require_no != undefined) {
          // console.log("click 2")
          setProductOptionValue2(
            resp.data.body.options_require_no.length > 1
              ? resp.data.body.options_require_no[0].product_option_value
              : [],
          );
        }
  ...
  setStates = () => {
    ...
    if (
      body.options_require_yes != undefined &&
      body.options_require_yes.length == 1
    ) {
      // console.log("click 4")
      setProductOptionValue(
        getImageObject(body.options_require_yes[0].product_option_value),
      );
    } else if (
      body.options_require_yes != undefined &&
      body.options_require_yes.length == 2
    ) {
      // console.log("click 3")
      setProductOptionValue(
        getImageObject(body.options_require_yes[0].product_option_value),
      ...
      body.options_require_yes != undefined &&
      body.options_require_yes.length == 3
    ) {
      // setProductOptionValue(getImageObject(body.options_require_yes[0].product_option_value))
      setProductOptionValue2(
        getImageObject(body.options_require_yes[1].product_option_value),
      );
  ...
  selectColor = item => {
    ...
    if(body.personalized_main_options_require_yes != undefined && body.personalized_main_options_require_yes.length == 5){
      arr = body.personalized_main_options_require_yes[3].product_option_value;
    }
  ...
  getImageObject = array => {
    ...
    arr.map((data, i) => {
      ...
      if (arr[i].image.uri == undefined) arr[i].image = {uri: data.image};
    });
    // console.log("click 5")
    return arr;
  };
  ...
  viewTypeSelect = item_val => {
    if (
      ...
      setImage(
        item_val.option_image_path != null ? item_val.option_image_path : image,
      );
      setPrice(item_val.price != false ? item_val.price : price);
    } else if (
      body.options_require_yes != undefined &&
      ...
      setPrice(item_val.price != false ? item_val.price : price);
      if (
        body.options_require_yes[1].product_option_value_data_child.length ==
        undefined
      ) {
        setTraySizes(
          body.options_require_yes[1].product_option_value_data_child[
            item_val.option_value_id
          ],
        );
        setIsSelect(true);
      }
      ...
      setTraySizes(
        body.options_require_yes[2].product_option_value_data_child[
          item_val.option_value_id
        ],
      );
      setIsSelect(true);
  ...
  selectOccasions = item_val => {
    occasions_type[occasions.indexOf(item_val.name)] == 1
      ? setShowDate(true)
      : setShowDate(false);

    setDesignImage(
      body.options_require_no[1].product_option_value_data_child[
        item_val.option_value_id
      ],
    );
    setImage(
  ...
  console.log("product option value => ", product_option_value_3);
  // console.log("stock =>  ", dropdown);
    // console.log("sub id =",subDropdownId[0].option_value_id)
    ...
    <SafeAreaView style={portraitStyles.screenBackgroundStackTab}>
      {item.success == undefined ? (
        ...
        <ImageBackground
          ...
          resizeMode="cover">
          <ScrollView
            style={portraitStyles.container}
            ...
            <View style={{flex: 1}}>
                ...
                <View style={portraitStyles.modalContainer}>
                  ...
                  <View style={{flexDirection: 'row'}}>
                    ...
                    <TouchableOpacity
                      ...
                      onPress={getCode}
                      disabled={spinner === false ? true : false}>
              ...
              <TouchableOpacity style={portraitStyles.otpButton} onPress={() => getCode()}>
                  ...
                  <TouchableOpacity
                    style={{padding: 5}}
                    onPress={() =>
                      navigation.navigate('signup', setModalVisible(false))
                    }>
                    <Text
                      style={{
                        fontSize: 18,
            ...
            <View style={portraitStyles.homeAccentContainer}>
              <View style={portraitStyles.productProfileContainer}>
                <TouchableOpacity>
                  <View style={portraitStyles.homeAccentImageContainer}>
                    <ImageLazyLoading
                      style={portraitStyles.homeAccentImage}
                ...
                <View style={portraitStyles.homeAccentTextContainer}>
                  ...
                  <Text style={portraitStyles.productProfilePrice}>
                    {price}
                  </Text>
                </View>
                <View style={portraitStyles.optionParentcontainer}>
                  {renderIf(body.handcrafted == 1)(
                    <View style={portraitStyles.optionContainer}>
                      <Image
                        style={portraitStyles.optionIcon}
                        source={require('../../assets/images/options_icons/handcrafted_icon.png')}
                      />
                      <Text style={portraitStyles.optionText}>
              ...
              {refrenceCode ? (
                <View style={portraitStyles.noteText}>
                  <View style={{paddingBottom: 10}}>
                    <Text style={portraitStyles.personalizeText}>
                      Please email us your Personalized Design at{' '}
                    </Text>
                    <Text style={portraitStyles.customerCareText}>
                      {' '}
                      customercare@craftslane.com{' '}
                    </Text>
                    <Text style={portraitStyles.personalizeText}>
                      {' '}
                      Artwork Specifications: Open Files in Illustrator or
                      CorelDraw
                    </Text>
                  </View>
                  ...
                  <View style={{paddingBottom: 10}}>
                    <Text style={portraitStyles.personalizeText}>
                      Once we have confirmed that your Personalized Design is
                      suitable in terms of Content and Quality, you will receive
                      a Design Reference Code.
                    </Text>
                    <Text
                      style={{
                        fontSize: 12,
                      ...
                      You will need to enter the Code in the section below to
                      place your order.
                    </Text>
                  </View>

                  <Text style={portraitStyles.personalizeText}>
                    Enter the Design Reference Code you received:
                  </Text>
                ...
                <View style={{padding: 10}}>
                  <SelectCountry
                    ...
                    onChange={item => {
                      ...
                      send_data[item.parent_product_option_id] =
                        item.product_option_value_id;
                      options_require[item.parent_product_option_id] =
                        item.product_option_value_id;
                ...
                <View>
                  ...
                  <View style={{padding: 10}}>
                    <SelectCountry
                      ...
                      imageField="image"
                      placeholder={
                        'Select a ' + body.options_require_yes[0].name
                      }
                      searchPlaceholder="Search..."
                      onChange={item => {
                        setImage(
                          item.option_image_path != null
                            ? item.option_image_path
                            : image,
                        );
                        send_data[item.parent_product_option_id] =
                          item.product_option_value_id;
                  ...
                  <View style={{padding: 10}}>
                    <SelectCountry
                      ...
                      imageField="image"
                      placeholder={
                        'Select a ' + body.options_require_yes[1].name
                      }
                      searchPlaceholder="Search..."
                      onChange={item => {
                        selectColor(item);
              ...
              {is_Select_color ? (
                <View style={{padding: 10}}>
                  <SelectCountry
                    ...
                    imageField="image"
                    placeholder={
                      'Select a ' + body.options_require_yes.length != 3
                        ? body.options_require_yes[1].name
                        : body.options_require_yes[2].name
                    }
                    searchPlaceholder="Search..."
                    onChange={item => {
                      setImage(
              ...
              {is_Select ? (
                <View style={{padding: 10}}>
                  <SelectCountry
                    ...
                    onChange={item_val => {
                      setImage(
                        item_val.option_image_path != null
                          ? item_val.option_image_path
                          : image,
                      );
                      setPrice(item_val.price != null ? item_val.price : price);
                      send_data[item_val.parent_product_option_id] =
                        item_val.product_option_value_id;
                      options_require[item_val.parent_product_option_id] =
                        item_val.product_option_value_id;
              ...
              {body.view_type == "7" && body.personalized_main_options_require_yes != undefined &&
              body.personalized_main_options_require_yes.length <=5  ?
                <View>
                {body.personalized_main_options_require_yes.map((item,ind)=>(
                  <>
                    { dropdown[ind] ?
                      <View style={{padding: 10}}>
                        <SelectCountry
                          ...
                          iconStyle={styles.iconStyle}
                          maxHeight={200}
                          data={Array.isArray(item.product_option_value_data_child) ? ind != 3 ? item.product_option_value : product_option_value_3 : subDropdownId}
                          valueField="product_option_value_id"
                          labelField="name"
                          imageField="image"
                          ...
                          onChange={item_val => {
                            ...
                            if(Array.isArray(item.product_option_value_data_child)){
                              ...
                              // console.log("before change =>",arr[ind+1]);
                              if(ind == 0)
                              newArr[1] = true;
                              else if(ind == 2)
                              newArr[3] = true;
                              // console.log(" after =>",arr[ind+1]);
                              setDropdown(newArr);
                              // console.log("new array =>",arr);
                            }
                            if(ind == 2){
                              selectColor(item_val);
                            }
                            setImage(
                              item_val.option_image_path != null ? item_val.option_image_path : image,
                            );
              ...
              body.personalized_main_options_require_yes.length == 5 ?
                ...
                {body.personalized_main_options_require_yes.map((item,ind)=>(
                  ...
                  <SelectCountry
                    ...
                    onChange={item => {
                      setImage(
                        item.option_image_path != null ? item.option_image_path : image,
                      );
              ...
              {body.options_require_no != undefined ? (
                <TouchableOpacity
                  ...
                  <MaterialIcons
                    name={toggle ? 'radio-button-on' : 'radio-button-off'}
                    size={25}
                    color={'#B48D56'}
                  />
                  <Text
                    ...
                    {body.options_require_yes != undefined ? 'How w' : 'W'}ould
                    you like us to Personalize it for you?
                  </Text>
                </TouchableOpacity>
              ) : (
                <></>
                    ...
                    {design_image.map((data, i) => (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          ...
                          send_data[design_image[i].parent_product_option_id] =
                            design_image[i].product_option_value_id;
                        }}
                        key={i}>
                        <Image
                          style={{height: 100, width: 100}}
                          source={
                            data.image !== undefined ? {uri: data.image} : null
                          }></Image>
                      </TouchableOpacity>
                    ))}
                  </ScrollView>
              ...
              {renderIf(showPersonalization && toggle)(
                <View style={portraitStyles.trayStyleContainer}>
                  <Text style={portraitStyles.headerTrayStyle}>
                  ...
                  <View style={portraitStyles.trayStyleChild}>
                    <View style={portraitStyles.containLabelAndInput}>
                      <TextInput
                        ...
                        autoComplete="none"
                        autoCorrect="none"
                        onChangeText={text => (send_data['name'] = text)}
                      />
                    </View>
                    {renderIf(show_date)(
                      <View style={portraitStyles.containLabelAndInput}>
                        <TextInput
                          ...
                          placeholderTextColor={'grey'}
                          defaultValue={
                            flag ? JSON.stringify(date).substring(1, 11) : ''
                          }
                        ...
                        <DatePicker
                          ...
                          onConfirm={date => {
                            ...
                            setDate(date);
                            setFlag(true);
                            send_data['date'] = JSON.stringify(date).substring(
                              1,
                              11,
                            );
                          }}
            ...
            {refrenceCode ? (
              <View style={portraitStyles.noteText}>
                ...
                <View>
                  <Text>
                    1. Craftslane will not be responsible for any design
                    elements that may get slightly modified as we try to place
                    your design on to the Product(s) selected{' '}
                  </Text>
                  <Text>
                    2. Craftslane reserves the right to decline any order which
                    may require us to work with designs or matter that are in
                    any way offensive.{' '}
                  </Text>
                </View>
              </View>
            ...
            <View style={portraitStyles.incDecButtonContainerProfile}>
              <Text
                style={
                  body.stock >= itemcnt
                    ? portraitStyles.quantityText
                    : portraitStyles.quantityTextFade
                }>
                Quantity:
              </Text>
              <View style={portraitStyles.cartIncDecContainer}>
                <TouchableOpacity
                  ...
                  style={
                    body.stock >= itemcnt
                      ? portraitStyles.decBtn
                      : portraitStyles.decBtnFade
                  }
                  disabled={body.stock + 1 >= itemcnt ? false : true}
                  onPress={() => (itemcnt > 1 ? setItemcnt(itemcnt - 1) : '')}>
                  <Text
                    style={
                      body.stock >= itemcnt
                        ? portraitStyles.decButton
                        : portraitStyles.decButtonFade
                    }>
                    -
                  </Text>
                </TouchableOpacity>
                <Text
                  style={
                    body.stock >= itemcnt
                      ? portraitStyles.incDecField
                      : portraitStyles.incDecFieldFade
                  }>
                  {itemcnt}
                </Text>
                <TouchableOpacity
                  activeOpacity={0.9}
                  style={
                    toggle ? portraitStyles.decBtn : portraitStyles.decBtnFade
                  }
                  disabled={body.stock >= itemcnt ? false : true}
                  onPress={() => setItemcnt(itemcnt + 1)}>
                  <Text
                    style={
                      body.stock >= itemcnt
                        ? portraitStyles.decButton
                ...
                {body.stock < itemcnt ? (
                  <Text style={portraitStyles.oopsText}>
                    {body.stock == 0
                      ? "Oops! We don't have this in stock at the present time."
                      : 'Oops! We have only ' +
                        body.stock +
                        ' units left in stock'}
                  </Text>
            ...
            {renderIf(out_of_stock)(
              <View style={portraitStyles.cartButtonContainer}>
                <Pressable
                  style={portraitStyles.cartbutton}
                  onPress={() => addToCart(product_id)}>
            ...
            {renderIf(!show_notify)(
              <View>
                ...
                <TouchableOpacity
                  activeOpacity={0.9}
                  style={portraitStyles.buttonContainer}
                  onPress={() => {
                    notifyUser();
                  }}
                  disabled={notifySpinner === false ? true : false}>
            ...
            {renderIf(show_notify_message)(
              <View style={portraitStyles.noteContainer}>
                <Text style={portraitStyles.notifySuccessText}>
                  Success: Thanks for showing interest in this product. we will
                  update you soon as this will update in stock.
                </Text>
              </View>,
            )}
            <View style={portraitStyles.overViewAndShippingPolicyContainer}>
              <TouchableOpacity
                activeOpacity={0.9}
                style={portraitStyles.overViewContainer}
                onPress={() => setPlusMinus(plus_minus == '-' ? '+' : '-')}>
              ...
              {plus_minus == '-' ? (
                <View style={portraitStyles.accordianContainer}>
                  <Text style={portraitStyles.accordianParagraph}>
                    {body.description.split('+++').length >= 0
                      ? body.description.split('+++')[0]
                      : ''}
                  </Text>
                  <Text style={portraitStyles.accordianParagraph}>
                    {body.description.split('+++').length > 1
                      ? body.description.split('+++')[1]
                      : ''}
                  </Text>
                  <Text style={portraitStyles.accordianParagraph}>
                    {body.description.split('+++').length > 2
                      ? body.description.split('+++')[2]
                      : ''}
                  </Text>
                  {body.attribute_groups.length > 0 ? (
                    body.attribute_groups[0].attribute.map((text, id) => (
                      <Text style={portraitStyles.accordianText} key={id}>
                        {`\u2022`} {text.text}
                      </Text>
                    ))
                  ) : (
              ...
              <TouchableOpacity
                activeOpacity={0.9}
                style={portraitStyles.shippingPolicyContainer}
                onPress={() =>
                  setSpPlusMinus(sp_plus_minus == '-' ? '+' : '-')
                }>
              ...
              {sp_plus_minus == '-' ? (
                <View style={portraitStyles.accordianContainer}>
                  <Text style={portraitStyles.shippingHeadings}>
                    Shipping Policy
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    Shipping within mainland India is Free!
                  </Text>
                  <Text style={portraitStyles.shippingHeadings}>Shipping</Text>
                  <Text style={portraitStyles.accordianText}>
                    We generally ship within 1 or 2 days after the payment has
                    been received
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    Application of a coat of Primer takes 2 days and Orders will
                    be dispatched within 3 - 4 days after the Order has been
                    placed
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    For Painted products kindly budget at least 4 - 5 days
                    extra, and Orders will be dispatched 5 - 6 days after the
                    Order has been placed
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    For Personalized products kindly budget 7 days extra, and
                    Orders will be dispatched 7 - 8 days after the Order has
                    been placed
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    Dispatches for the day are boxed by 5.00 p.m. and are picked
                    up at 6.00 p.m.
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    There are no dispatches on the weekends or public holidays
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    Orders once placed cannot be cancelled under any
                    circumstance
                  </Text>
                  <Text style={portraitStyles.shippingHeadings}>Handling</Text>
                  <Text style={portraitStyles.accordianText}>
                    There is a small Handling Charge of Rs.99 on Orders less
                    than Rs.250 (excluding GST)
                  </Text>
                  <Text style={portraitStyles.shippingHeadings}>Delivery</Text>
                  <Text style={portraitStyles.accordianText}>
                    Delivery generally takes 5 – 6 working days, once we ship,
                    based on the delivery locations
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    There is an extra charge for Expedited Delivery which can
                    only be assessed once the goods are ready for dispatch
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    In the event that Expedited Delivery is required, the extra
                    amount payable will be communicated, and goods dispatched
                    once the extra amount has been credited into our Bank
                    Account
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    Our logistics partners are FedEx and Speed Post
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    You may track your shipments on www.fedex.com/in and
                  ...
                  <Text style={portraitStyles.accordianText}>
                    For International Shipping, the Product Prices will be as
                    displayed on the Website
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    The Prices will be visible in USD, Pound Sterling and Euro
                    based on your IP address
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    There are no GST (Local Sales Taxes) applicable on
                    International Orders
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    The Currency is dynamically computed on a daily basis
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    The minimum Order Value is INR 1,500 or approximately USD 20
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    The Freight will be computed once the order has been Boxed
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    An Invoice for the Freight amount will be raised and emailed
                    separately through our secure Payment Gateway CCAvenue
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    The Order will be dispatched once we have received the
                    Freight payment
                  </Text>
                  <Text style={portraitStyles.accordianText}>
                    On Orders with a value over INR 1,00,000, we would request
                    you to please email us at customercare@craftslane.com
                  </Text>
                </View>
              ) : (
                <></>
