...
export default class AllProducts extends PureComponent {
  ...
  searchProduct = async val => {
    val
      ...
      : this.setState({resultPage: 0, result: []});
    ...
    if (this.state.all_data.status == 200) {
      ...
      this.setState({
        result: this.state.result.concat(this.state.all_data.products),
      });
    } else {
      showMessage({
  ...
  async categoryArray() {
    ...
    let res = await axios.get(
      this.state.data.url +
        'customcateautosuggestion/index&key=' +
        this.state.data.key +
        '&token=' +
        this.state.data.token,
    );
  ...
  async getUsers() {
    ...
    // console.log("Before Increment =>" , this.state.currentPage);
    this.setState({currentPage: this.state.currentPage + 1});

    // console.log("Afters Increment =>" , this.state.currentPage)
    ...
    if (this.state.all_data.status == 200) {
      this.setState({
        users: this.state.users.concat(this.state.all_data.products),
      });
    } else {
  ...
  renderItem = ({item}) => {
    return (
      <TouchableOpacity
        activeOpacity={0.9}
        style={portraitStyles.cartProductContainer}
        onPress={() =>
          this.props.navigation.push('homeaccent', {cat: '', id: item.id})
        }>
        <View style={portraitStyles.cartImageContainer}>
          {/* {console.log(item)} */}

          <ImageLazyLoading
            style={portraitStyles.cartImage}
                ...
                {/* <TouchableOpacity
                  ...
                  <View
                    ...
                    <MaterialCommunityIcons
                      ...
                      color={'white'}
                    />
                    <Text style={{fontSize: 18, padding: 3, color: 'white'}}>
                      Filter
                    </Text>
                  </View>
                </TouchableOpacity> */}

                {renderIf(this.state.isShowForm)(
                  <SafeAreaView style={portraitStyles.screenBackground}>
                    <View style={portraitStyles.filterFormInput}>
                      <TextInput
                        ...
                        onChangeText={text =>
                          this.setState({product: text, onChange: text})
                        }
                      />
                    </View>
                    ...
                    <View style={{padding: 10}}>
                      <Dropdown
                        ...
                        placeholder={
                          !this.state.isFocus ? 'Select category' : '...'
                        }
                    ...
                    <TouchableOpacity
                      onPress={() => this.searchProduct(false)}
                      activeOpacity={0.9}
